...
/**
 * Optimization & Security: Map to store in-flight promises for concurrent request coalescing.
 * Prevents "thundering herd" issues where multiple requests for the same data trigger redundant API calls.
 */
  ...
  try {
    ...
    if (!player) {
      ...
      if (pendingRequests.has(sanitizedAllyCode)) {
        ...
        // Optimization & Security: Store the pending promise in the Map
