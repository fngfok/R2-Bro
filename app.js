...
// Optimization: Map to track and coalesce concurrent requests to prevent thundering herd
// Security: Essential for DoS protection and ensuring application stability
...
app.get('/player/:allyCode', rateLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!player) {
      // Optimization/Security: Check if there is already a pending request for this ally code to coalesce concurrent calls
      // This implements concurrent request coalescing to protect against Thundering Herd DoS.
      if (pendingRequests.has(sanitizedAllyCode)) {
        ...
        // Optimization/Security: Store the promise in the Map to coalesce subsequent concurrent requests
