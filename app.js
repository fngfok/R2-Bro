...
// Optimization: Map to store pending API requests to prevent thundering herd (request coalescing)
...
app.get('/player/:allyCode', rateLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!player) {
      ...
      // We use a single .get() lookup to avoid hashing overhead of has() + get()
      ...
      if (pending) {
        ...
        const fetchPromise = comlink.getPlayer(sanitizedAllyCode)
          ...
          .finally(() => {
            // Optimization: Remove from pending map as soon as request completes
            pendingRequests.delete(sanitizedAllyCode);
          });

        // Optimization: Store the promise before any await to avoid race conditions
