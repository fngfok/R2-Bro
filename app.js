...
app.get('/player/:allyCode', rateLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!player) {
      // Optimization: Use a single Map.get() to check for pending requests to avoid double-hashing overhead
      ...
      if (pending) {
        ...
        // Optimization: Store the promise in the map before any await to prevent race conditions
