...
// Security: Use a Map to coalesce concurrent requests and prevent 'thundering herd' DoS issues
...
app.get('/player/:allyCode', rateLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!player) {
      // Security/Optimization: Check if there is already a pending request for this ally code to coalesce concurrent calls (Thundering Herd prevention)
      if (pendingRequests.has(sanitizedAllyCode)) {
        ...
        const fetchPromise = comlink.getPlayer(sanitizedAllyCode)
          ...
          .finally(() => {
            // Security/Optimization: Ensure the promise is removed from the map regardless of success or failure
            pendingRequests.delete(sanitizedAllyCode);
          });

        // Security/Optimization: Store the in-flight promise to coalesce concurrent requests
