...
## 2026-05-15 - Thundering Herd Protection Restoration
**Learning:** The `pendingRequests` Map is critical for concurrent request coalescing in `app.js`. Removing it while keeping the logic that references it causes a `ReferenceError` and breaks thundering herd protection.
**Action:** Always verify that all module-level variables required by route handlers are present and correctly documented to prevent accidental removal during cleanup.
